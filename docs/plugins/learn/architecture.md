...
Session state flows through files in `screen_dir/` — never through conversation history. `learn:micro` is the only actor that generates lesson content. The router (`learn`) detects intent and makes routing decisions explicit to the user. The visual server is a separate process that watches `screen_dir/` and serves the newest file — it has no direct connection to Claude.
...
User
 │
 └─ /learn {topic} ──────────────────────────────────────────────────────────
     Reads: LEARN_DEBUG env var, .learn/prefs.json (V2)
...
| `screen_dir/waiting.html` | learn router | Shown between lessons while a new lesson is being generated. |
| `state_dir/events` | visual server | Quiz interactions written as JSON lines. Append-only. |
| `.learn/prefs.json` | learn router | Expertise level + user preferences. V2 only — not read in V1. |

---

## How the visual server works

The visual server is a file-watcher HTTP server. It is a separate process, started independently with `learn serve`, not managed by any skill.
...
**Auto-exit:** The server exits automatically after 30 minutes with no lesson file activity. The user can restart it at any time with `learn serve` — the server is stateless and resumes correctly.
...
| Failure | Behaviour |
|---------|-----------|
| Port conflict | Auto-increment to next free port. Log actual URL used: `learn serve: listening on http://127.0.0.1:7338` |
| Server not running when lesson is ready | Skip visual render. Show error: `"Visual server is not running — start it with 'learn serve'"` |
...
3. Add the skill to the routing table in `skills/learn/SKILL.md`
4. Add a feature file to `docs/plugins/learn/specs/{name}.feature`
5. Update `docs/plugins/learn/architecture.md` to include the new skill in the flow diagram
6. Update `docs/plugins/learn/_index.md` skill table
...
- **Scope classifier always surfaces the routing decision.** The router never silently sends the user to `learn:micro`. If it routes, it says so. If it asks a clarifying question, it waits for the answer.
- **`LEARN_DEBUG=1` enables routing decision log.** When set, the router outputs its full classification reasoning before acting.
