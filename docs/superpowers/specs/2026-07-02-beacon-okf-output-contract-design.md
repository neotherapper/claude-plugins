...
- ✅ **T5-m1 / marker-JSON quoting — RESOLVED** (branch `fix/beacon-okf-hardening`): only `scaffold.sh`'s marker writer was the fail-open path — it `printf '%s'`-interpolated `OUTPUT_ROOT` into a JSON string, so a `"`/`\` in the path produced malformed JSON that `json.load` in `okf-gate.sh` rejected, no-opping the gate. Now built with `json.dumps` (path passed as argv). `okf-gate.sh`'s *read* was already argv-safe (`$MARK` is `sys.argv[1]`, never interpolated) — left unchanged. Covered by a round-trip test (scaffold a quote-bearing path → `json.load(marker)['output_root']` equals it).
...
- ✅ **T6-m1 — RESOLVED** (branch `fix/beacon-okf-hardening`): the `[LEGACY-WORKSPACE]` detection now runs deterministically inside `scaffold.sh` — on the default path only, it emits `[LEGACY-WORKSPACE:docs/research/{slug}]` when a pre-0.7.0 folder exists; SKILL.md reacts to the signal instead of detecting in prose. Keyed on "caller supplied no `OUTPUT_ROOT`" (not `OUTPUT_ROOT_OVERRIDDEN`), so an explicit caller path — incl. SKILL.md's own `docs/research/{slug}` example — is never nagged. Covered by positive + negative tests.
