 ...
 *   Referer/Origin URL path, never from the request body. Any `plugin` or
...
export async function handleEventPost(
  ...
  // Derive plugin + surface from Referer or Origin path (server-side only — body field ignored).
  ...
  try {
    payloadStr = Buffer.from(token.slice(0, dot), 'base64url').toString('utf8');
  } catch {
    res.writeHead(403, securityHeaders());
    res.end('Forbidden');
    return;
  ...
  const [tokenPlugin, tokenSurface, nonce] = parts;
  if (!tokenPlugin || !tokenSurface || !nonce) {
    res.writeHead(403, securityHeaders());
    res.end('Forbidden');
    return;
  }
  if (tokenPlugin !== plugin || tokenSurface !== surfaceId) {
    res.writeHead(403, securityHeaders());
    res.end('Forbidden');
    return;
  }
