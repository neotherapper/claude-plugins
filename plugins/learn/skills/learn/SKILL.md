...
Entry point for the `learn` plugin. Classify the user's learning intent, determine scope, and route to the correct sub-skill. Always surface the routing decision — never silently reroute.

## Routing table

| User input | Action |
|---|---|
| "teach me [concept]" | Scope check → route to learn:micro |
...
Before routing to learn:micro, classify whether the topic is a single concept or a broad area.

**Single concept (route directly to learn:micro):**
...
If the user picks option 1: suggest the best entry concept for the topic and invoke learn:micro with it.
...
If `LEARN_DEBUG=1` is set in the environment, print the routing decision before invoking any sub-skill:

```
[learn:router] intent=micro topic="{topic}" level={level} scope={single|broad}
```

## What this skill does NOT handle

- Lesson generation — delegated to learn:micro
