...
Load `references/brief-questions.md` for the exact question wording and validation rules.
...
Create workspace directory `posts/{slug}/`. Load `references/workspace-schema.md` for the complete file contract — every file created must be documented there.
...
Load `references/layout-templates.md` for section templates and word-count ranges.
...
Load `references/scoring-rubric.md` if the orchestrator surfaces a routing decision that requires user input (escalation when a dimension scores < 50, or max iterations reached).
...
Load `references/distribution-guide.md` to format the distribution copy display correctly (respecting character limits per platform).

If Turso is enabled in `.draftloom/config.json`, load `references/turso-setup.md` to confirm the post was synced.
...
Load these files at the step indicated — do not preload all at once:

- **`references/brief-questions.md`** — exact wording for all 7 brief questions (4 mandatory, 3 optional), validation rules, and the brief.md output format (load at Step 2)
