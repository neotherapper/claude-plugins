...
Beacon ships with tech packs for WordPress, Next.js, Nuxt, Django, Rails, Shopify, Astro, Laravel, Ghost, Express, React, Sylius, PrestaShop, OpenCart, Shopware, BigCommerce, Wix, Squarespace, Ecwid, Big Cartel, Square Online, Joomla, Webflow, Drupal, Magento, WooCommerce, TYPO3, SolidJS, SvelteKit, React Native, Vercel, Spring Boot, ASP.NET Core, NestJS, Symfony, Contentful, Sanity, Storyblok, Contentstack, AEM, Sitecore, Optimizely, and Umbraco. Each pack tells Beacon where to look for APIs, config values, auth patterns, and known public endpoints on sites using that framework.
