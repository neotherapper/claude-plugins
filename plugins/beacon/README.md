...
Beacon ships with tech packs for WordPress, Next.js, Nuxt, Django, Rails, Shopify, Astro, Laravel, Ghost, Express, React, Sylius, PrestaShop, OpenCart, Shopware, BigCommerce, Wix, Squarespace, Ecwid, Big Cartel, Square Online, Joomla, Webflow, and Drupal. Each pack tells Beacon where to look for APIs, config values, auth patterns, and known public endpoints on sites using that framework.
